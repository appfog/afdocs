...
AppFog may not properly detect a Sinatra app if it uses Bundler. AppFog greps for `require "sinatra"` in the `*.rb` files, and picks that file as the main Sinatra app file. If you use Bundler to load Sinatra, AppFog may not recognize your app. You can put a comment in your app’s main file like this:
