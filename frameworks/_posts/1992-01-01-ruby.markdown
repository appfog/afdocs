...
AppFog may not properly detect a Sinatra app if it uses Bundler. AppFog greps for `require "sinatra"` in the `*.rb` files, and picks that file as the main Sinatra app file. 
