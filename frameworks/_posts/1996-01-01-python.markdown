...
TKTKTK You can define the package prequisites of your Python app in a top-level [requirements.txt]() file, which `pip` uses to install dependencies. AppFog never caches package dependencies and instead downloads and installs them every time you update or restart your app. 

TKTKTK AppFog uses pip and virtualenv. 
...
First, add the following to the beginning of `wsgi.py`:
...
Then add the following to the end, right before `if __name__ == '__main__':`
...
We also need to install `pymongo`:
