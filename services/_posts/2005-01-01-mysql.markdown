...
First grab the phpMyAdmin source from our GitHub repo: 
...
Then push the code up to a new AppFog app (making sure to bind it to the MySQL service you want to connect to):

{: .prettyprint}
    $ af push myadmin-example
    Would you like to deploy from the current directory? [Yn]:
    Detected a PHP Application, is this correct? [Yn]:
    Application Deployed URL [myadmin-example.aws.af.cm]:
    Memory reservation (128M, 256M, 512M, 1G, 2G) [128M]:
    How many instances? [1]:
    Bind existing services to 'myadmin-example'? [yN]: y
    1: mysql-example
    Which one?: 1
    Bind another? [yN]:
    Create services to bind to 'myadmin-example'? [yN]:
